...
void Graph::drawImage(std::wstring *fWPath,int szClock)
{
	...
	int edgeWidthDivider = 255 / (cfg->gMaxEdgeWidth - cfg->gMinEdgeWidth + 1);
	...
	for (unsigned int x = 0;x < edges.size();x++)
	{
		...
		Gdiplus::Pen p(Gdiplus::Color((int)alphaFinal,cfg->iEdgeColor.r-eiDiffR,cfg->iEdgeColor.g-eiDiffG,cfg->iEdgeColor.b-eiDiffB),((float)((weightStrength >= 255 ? 254 : weightStrength) / edgeWidthDivider) + cfg->gMinEdgeWidth)); 
...
void Graph::randomizeNodePositions()
{
	for (std::map<std::string,Node*>::iterator it = nodes.begin(); it != nodes.end(); it++)
	{
		Node *node = it->second;
		node->setX(rand32(20000) / 10000.0);
		node->setY(rand32(20000) / 10000.0);
	}
}